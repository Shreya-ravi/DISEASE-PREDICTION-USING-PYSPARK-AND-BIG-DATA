...
file_path = "hdfs://localhost:9000/user/bda/project/Disease_symptom_and_patient_profile_dataset_100mb.csv"
df = spark.read.csv(file_path, header=True, inferSchema=True)
output_path = "hdfs://localhost:9000/user/bda/project/output1"
...
for trained_model, name in zip(trained_models, model_names):
    try: 
        ...
        predictions.select("features", "Outcome Variable", "prediction").write.mode("append").parquet(output_path)
        #predictions.select("features", "Outcome Variable", "prediction").write.mode("append").json(output_path)
